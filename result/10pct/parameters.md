# parameters — ProtoPFormer (crop 10pct)

> 학습 하이퍼파라미터(재현용). 실행: `scripts/train_bogonet.sh` → `main.py`. split02, leakage 0.

## 재현 명령
```bash
bash scripts/train_bogonet.sh 64 150 10pct                      # 1차 150ep
bash scripts/train_bogonet.sh 64 200 10pct <checkpoint-latest>  # 200ep까지 resume (공정 비교)
```
...
|---|---|
| batch_size | 64 |
| epochs | 200 (150 학습 후 resume으로 +50) |
| optimizer | AdamW |
| weight_decay | 0.05 |
| scheduler | cosine |
